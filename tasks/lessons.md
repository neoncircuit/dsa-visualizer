...
### 44. Null-Check Is Not Enough When State Can Be Invalid But Non-Null
- **What happened**: `initGenerator()` checked `if (!currentMaze)` before running fallback maze generation. But `switchVizMode()` had already assigned `currentMaze` to a fresh all-walls grid (via `buildMazeGrid()`). The check passed (the variable was not null), so the pathfinder ran on an all-walls grid and terminated immediately.
- **Rule**: When a state variable can exist in both a "valid" and "placeholder" state, a null check alone is insufficient. Track readiness explicitly with a boolean flag (e.g., `mazeReady`). Set the flag to `true` only after the state is fully initialised, and set it to `false` whenever the state is replaced with a placeholder.
- **How to apply**: Add a companion boolean flag alongside any state that has a multi-stage lifecycle: `let currentMaze = null; let mazeReady = false;`. Set `mazeReady = true` only in the function that produces the fully-initialised value. Check `if (!currentMaze || !mazeReady)` in consumers that depend on a fully-initialised value. Also ensure `reset()` calls the correct generator (e.g., `generateMaze()` not `generateArray()`) for the active algorithm family.

### 45. Pre-Initialising State That a Generator Algorithm Will Discover Itself Creates Isolation Bugs
- **What happened**: `buildMazeGrid()` pre-marked both (1,1) and (rows-2, cols-2) as PASSAGE. The maze generators use `getMazeNeighbors()` which filters `grid[nr][nc] === WALL` to find unvisited cells. Because the endpoint was already PASSAGE, no neighbor ever targeted it for carving, and the wall between it and its nearest carved cell remained intact. The endpoint was a passage cell surrounded entirely by walls -- unreachable from the rest of the maze.
- **Rule**: When a procedural generator discovers and carves cells itself, do not pre-initialise those cells in the grid builder. The generator's own discovery logic must be the sole authority on which cells are carved and when. Pre-marking cells that the generator is supposed to find creates a mismatch between the generator's view of the world (filtering for WALL) and the actual grid state (cell already PASSAGE).
- **How to apply**: `buildMazeGrid()` should return a pure all-walls grid. Each maze generator marks (1,1) as its starting cell and carves from there. The endpoint is reached naturally as part of the perfect maze. Visual markers (START=2, END=3) are applied by the renderer via `refreshStartEnd()`, not stored in the data grid.

### 46. In Lazy-Deletion Search Algorithms, Pre-Adding the Start Node to the Visited Set Skips It
