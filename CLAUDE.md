...
- **Dev Server**: Vite via `pnpm dev`
- **Package Manager**: pnpm
- **Containerization**: Docker + docker-compose (multi-stage build with nginx)
- **Architecture**: Static SPA served by Vite in dev, nginx in production
...
- **CICD**: CI/CD Pipelines are important to ensure that code quality is not compromised, so we need to run checks locally until they all clear before pushing.
